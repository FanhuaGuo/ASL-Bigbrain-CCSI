...
Data1_PScs_bil_corrected = subfun_GlobalMeanOffsetCorrection(Data1_PScs_bil);
...
Data2_PScs_bil_corrected = subfun_GlobalMeanOffsetCorrection(Data2_PScs_bil);
...
CBF1_meanPScs_bil = mean(Data1_PScs_bil_corrected,2);
CBF1_meanPScs_uni = mean(Data1_PScs_uni,2);
CBF2_meanPScs_bil = mean(Data2_PScs_bil_corrected,2);
...
% write to SourceData EDFig4a
outSD = CBF2_meanPScs_bil;
...
tmp = table(mean(Data2_PScs_uni,2), std(Data2_PScs_uni')'/sqrt(size(Data2_PScs_uni,2)), atlas_info.Lobe(1:180), atlas_lobeindex, p',...
...
yline(bias, 'r-', 'LineWidth', 0.5, 'Label', 'Bias'); 
yline(loa_upper, 'b--', 'LineWidth', 0.5, 'Label', '+1.96 SD');
yline(loa_lower, 'b--', 'LineWidth', 0.5, 'Label', '-1.96 SD');
...
% write to SourceData EDFig4c
outSD = [CBF1_meanPScs_bil/2+CBF2_meanPScs_bil/2 CBF1_meanPScs_bil-CBF2_meanPScs_bil];
outSD(CBFsnr_mean_bil<=4,:) = nan;
...
% write to SourceData EDFig4c
outSD = [CBF1_ValPC1_bil/2+CBF2_ValPC1_bil/2 CBF1_ValPC1_bil-CBF2_ValPC1_bil];
outSD(CBFsnr_mean_bil<=4,:) = nan;
...
%% revision compare retest 14 and test other 16
retest_subjs = [1:3 6 8:9 13 15:17 19:22];
whole_subjs = [1:9 13 15:22 32 36:46];
retest_mask_subjs = zeros(numel(whole_subjs),1);
for i = 1:numel(whole_subjs)
    for j = 1:numel(retest_subjs)
        if whole_subjs(i) == retest_subjs(j)
            retest_mask_subjs(i) = 1;
        end
    end
end
retest_mask_subjs = retest_mask_subjs > 0;

CCSI_16 = mean(Data_PScs_bil_correct(CBFsnr_mean_bil>4,~retest_mask_subjs),2)-0.2;
CCSI_14_1 = mean(Data_PScs_bil_correct(CBFsnr_mean_bil>4,retest_mask_subjs),2);
% CCSI_14_1 = mean(Data1_PScs_bil_corrected,2);
CCSI_14_2 = mean(Data2_PScs_bil_corrected(CBFsnr_mean_bil>4,:),2);

corr(CCSI_16,CCSI_14_1)
corr(CCSI_16,CCSI_14_2)
corr(CCSI_14_1,CCSI_14_2)
% CBFsnr_mean_bil

figure('Color',[1 1 1],'Position',[0 0 800 300],'Units','pixels');
subplot(131); hold on;
subfun_plot_scatter_regression(CCSI_14_1 , CCSI_16 , [-0.5 1.1] , [-0.5 1.1]);
xlabel('CCSI - 14 test')
ylabel('CCSI - other 16')
subplot(132); hold on;
subfun_plot_scatter_regression(CCSI_14_2 , CCSI_16 , [-0.5 1.1] , [-0.5 1.1]);
xlabel('CCSI - 14 retest')
ylabel('CCSI - other 16')
subplot(133); hold on;
subfun_plot_scatter_regression(CCSI_14_1 , CCSI_14_2 , [-0.5 1.1] , [-0.5 1.1]);
xlabel('CCSI - 14 test')
ylabel('CCSI - 14 retest')
