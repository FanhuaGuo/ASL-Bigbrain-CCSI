function [ PS1_cs ] = subfun_ProfileSimilarity( DataScan , DataBB )
%% profile similarity
[N_region, N_layer] = size(DataBB);

PS1_cs = [];
for k = 1:numel(DataScan)
    % Normalize each region's vector
    norm_Data = zscore(DataScan{k}(:,1:end-1)')';
    norm_CellDensity = zscore(DataBB(:,1:end-1)')';
